...
@inline function augmented_kernel_size(grid::Distributed)
    Nx, Ny, _ = size(grid)
...
@inline function full_offsets(grid::Distributed)
    Hx, Hy, _ = halo_size(grid)
...
@inline function partitioned_halos(old_halos, step_halo, grid)
