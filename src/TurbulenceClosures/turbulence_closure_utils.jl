...
@inline calc_νᶜᶜᶜ(i, j, k, grid, closure, clock, fields, buoyancy) = zero(grid)
@inline calc_κᶜᶜᶜ(i, j, k, grid, closure, clock, fields, buoyancy, tracer_index) = zero(grid)
...
@kernel function calculate_nonlinear_tracer_diffusivity!(κₑ, grid, closure, args...)
    i, j, k = @index(Global, NTuple)
    @inbounds κₑ[i, j, k] = calc_κᶜᶜᶜ(i, j, k, grid, closure, args...)
end
