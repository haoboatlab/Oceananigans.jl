...
@inline tendency_kernel_size_aux(grid, ::Val{:interior})   = size(grid) 
@inline tendency_kernel_offset_aux(grid, ::Val{:interior}) = (0, 0, 0)

@inline tendency_kernel_size_aux(grid, ::Val{:west})   = @inbounds (1, size(grid, 2)+add2(halo_size(grid, 2)), size(grid, 3)+add2(halo_size(grid, 3)))
@inline tendency_kernel_size_aux(grid, ::Val{:south})  = @inbounds (size(grid, 1), 1, size(grid, 3))
@inline tendency_kernel_size_aux(grid, ::Val{:bottom}) = @inbounds (size(grid, 1), size(grid, 2), 1)
...
@inline tendency_kernel_offset_aux(grid, ::Val{:west})   = @inbounds (-1, -1, -1)
@inline tendency_kernel_offset_aux(grid, ::Val{:east})   = @inbounds (size(grid, 1), -1, -1)
@inline tendency_kernel_offset_aux(grid, ::Val{:south})  = @inbounds (0,             -1, -1)
@inline tendency_kernel_offset_aux(grid, ::Val{:north})  = @inbounds (0,             size(grid, 2), -1)
@inline tendency_kernel_offset_aux(grid, ::Val{:bottom}) = @inbounds (0,             0,             -1)
@inline tendency_kernel_offset_aux(grid, ::Val{:top})    = @inbounds (0,             0,             size(grid, 3))
